...
trait GBDB_InstanceSchemaTrait {
    ...
    /**
     * handles set schema constraint.
     *
     * @param string $database value.
     * @param string $table value.
     * @param string $column value.
     * @param string $constraint value.
    ...
    public static function setSchemaConstraint(string $database, string $table, string $column, string $constraint, mixed $value = true): bool {
        ...
        else if ($constraint === "required") $schema["columns"][$column]["required"] = (bool)$value;
        else if ($constraint === "unique") $schema["columns"][$column]["unique"] = (bool)$value;
        else if ($constraint === "auto_increment") $schema["columns"][$column]["auto_increment"] = (bool)$value;
        else if ($constraint === "sequence") $schema["columns"][$column]["sequence"] = (string)$value;
        else if ($constraint === "generated") $schema["columns"][$column]["generated"] = $value;
        else if ($constraint === "check") $schema["columns"][$column]["check"] = is_array($value) ? $value : [];
    ...
    /**
     * handles define migration.
     *
     * @param string $database value.
     * @param string $table value.
     * @param string $name value.
     * @param array $operations value.
     *
     * @return bool result.
     */
    ...
    /**
     * handles typed compare.
     *
     * @param string $database value.
     * @param string $table value.
     * @param string $column value.
     * @param mixed $left value.
     * @param string $op value.
     * @param mixed $right value.
     *
     * @return bool result.
     */
    ...
    private static function prepareSchemaRow(string $database, string $table, array $row, array $rows = [], ?int $excludeId = null, bool $writeGenerated = true): array {
        ...
        foreach (($schema["columns"] ?? []) as $col => $def) {
            ...
            if ($writeGenerated && (!$exists || $value === "" || $value === null || $value === "-header-")) {
                ...
                else if ((string)($def["sequence"] ?? "") !== "") $value = self::nextSchemaSequence($schema, (string)$def["sequence"]);
                else if (($def["generated"] ?? null) === "uuid" || ($def["id_generator"] ?? "") === "uuid") $value = self::uuidValue();
                else if (($def["generated"] ?? null) === "ulid" || ($def["id_generator"] ?? "") === "ulid") $value = self::ulidValue();
                else if (($def["generated"] ?? null) === "snowflake" || ($def["id_generator"] ?? "") === "snowflake") $value = self::snowflakeId();
                else if (($def["generated"] ?? null) === "distributed" || ($def["id_generator"] ?? "") === "distributed") $value = self::distributedId();
                else if (array_key_exists("default", $def)) $value = self::schemaDefaultValue($def["default"]);
            }
    ...
    private static function castSchemaValue(mixed $value, string $type, array $def = []): array {
        ...
        else if ($type === "int") { if (!is_numeric($value)) return ["ok" => false]; $value = (int)$value; }
        else if ($type === "float" || $type === "decimal") { if (!is_numeric($value)) return ["ok" => false]; $value = (float)$value; }
        else if ($type === "bool") $value = filter_var($value, FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE);
        else if ($type === "timestamp") { if (!is_numeric($value)) return ["ok" => false]; $value = (int)$value; }
        else if (in_array($type, ["datetime", "date", "time"], true)) {
