...
trait GBDB_MaintenanceTrait {
     ...
     * @param string $database value.
     * @param string $table value.
     * @param string|array $column value.
    ...
    /**
     * handles create unique index.
     *
     * @param string $database value.
     * @param string $table value.
     * @param string|array $columns value.
     *
     * @return bool result.
     */
    ...
    /**
     * handles create composite index.
     *
     * @param string $database value.
     * @param string $table value.
    ...
    public static function rebuildIndexes(string $database, string $table): bool {
        ...
        return (bool)self::withTableLock($lockFile, function () use ($file, $metaFile, $appendFile) {
            $meta = self::readMeta($metaFile);
            $defs = is_array($meta['index_definitions'] ?? null) ? $meta['index_definitions'] : [];
            $legacyIndexes = is_array($meta['indexes'] ?? null) ? array_values(array_filter($meta['indexes'], fn($column) => (string)$column !== '' && (string)$column !== 'id')) : [];

            if (empty($defs) && empty($legacyIndexes)) {
                return true;
            }
            ...
            if (empty($defs)) {
                foreach ($legacyIndexes as $column) {
