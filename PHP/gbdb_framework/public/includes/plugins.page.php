...
if (!is_dir($disabled)) @mkdir($disabled, 0777, true);
$msg = '';
$msgType = 'ok';
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    if (!GreenQLUIv2Helper::checkCsrf((string)($_POST['csrf'] ?? ''))) {
        ...
        if ($act === 'delete' && $file !== '' && str_ends_with($file, '.php')) { @unlink($dir . '/' . $file); @unlink($disabled . '/' . $file); $msg = 'Plugin gelöscht.'; }
        if ($act === 'disable' && is_file($dir . '/' . $file)) { @rename($dir . '/' . $file, $disabled . '/' . $file); $msg = 'Plugin deaktiviert.'; }
        if ($act === 'enable' && is_file($disabled . '/' . $file)) { @rename($disabled . '/' . $file, $dir . '/' . $file); $msg = 'Plugin aktiviert.'; }
    }
}

$plugins = [];
foreach (glob($dir . '/*.php') ?: [] as $f) $plugins[] = ['file' => basename($f), 'active' => true, 'size' => filesize($f)];
foreach (glob($disabled . '/*.php') ?: [] as $f) $plugins[] = ['file' => basename($f), 'active' => false, 'size' => filesize($f)];
    ...
    <main class="gbdbui-wide">
        ...
        <section class="gbdbui-page-grid">
            <div class="gbdbui-panel span-4">
                <div class="gbdbui-panel-head"><div><h2>Upload</h2><p>Nur PHP-Plugin-Dateien werden angenommen.</p></div></div>
            ...
            <div class="gbdbui-panel span-8">
                <div class="gbdbui-panel-head">
                    <div><h2>Installierte Plugins</h2><p><?= count($plugins) ?> Plugin-Datei(en) gefunden.</p></div>
                    <span class="gbdbui-pill">/plugins</span>
                </div>
                <?php if ($plugins): ?>
                    <div class="gbdbui-table-wrap">
                        <table class="gbdbui-table">
                            <thead><tr><th>Datei</th><th>Status</th><th>Größe</th><th>Aktionen</th></tr></thead>
                            <tbody>
                            <?php foreach ($plugins as $p): ?>
                                <tr>
                                    <td><code><?= gbdbui_e($p['file']) ?></code></td>
                                    <td><span class="gbdbui-pill <?= $p['active'] ? 'ok' : 'bad' ?>"><?= $p['active'] ? 'aktiv' : 'deaktiviert' ?></span></td>
                                    ...
                                    <td>
                                        <form method="post" class="gbdbui-action-row" style="margin:0">
                                            ...
                                            <button class="secondary" name="action" value="<?= $p['active'] ? 'disable' : 'enable' ?>" type="submit"><?= $p['active'] ? 'Deaktivieren' : 'Aktivieren' ?></button>
                                            <button class="danger" name="action" value="delete" type="submit" onclick="return confirm('Plugin löschen?')">Löschen</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
