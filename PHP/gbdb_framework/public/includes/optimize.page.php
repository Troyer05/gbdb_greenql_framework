...
<main class="gbdbui-wide legacy-tool-page">
    <section class="gbdbui-hero legacy-tool-hero">
        ...
        <h1>GBDB Optimize</h1>
        <p>
            Compact tables, inspect append/meta state and reduce write overhead after many inserts, edits and deletes.
                ...
                <label>
                    ...
                    <select name="table">
                        ...
                        <?php foreach ($tables as $table): ?>
                            <option value="<?= h($table) ?>" <?= ($table === $selTable ? 'selected' : '') ?>><?= h($table) ?></option>
                        <?php endforeach; ?>
                    </select>
                </label>
    ...
    <section class="gbdbui-panel legacy-note-panel">
        <div class="gbdbui-panel-head compact">
            <div>
                <h3>Tipp</h3>
                <p>Nach vielen Inserts, Edits oder Deletes lohnt sich ein Compact-Lauf, um Base-Dateien sauber neu zu schreiben.</p>
            </div>
        </div>
    </section>
</main>
