...
/** Liefert einen kleinen Request-/Session-Cache fuer teure UI-Listen. */
...
/** Speichert einen kleinen UI-Cachewert. */
...
/** Gibt Tabellen einer DB/Instanz gecached zurück. */
...
/** Leert den kleinen UI-Cache nach Strukturänderungen. */
function gbdbui_cache_clear(): void {
    if (session_status() !== PHP_SESSION_ACTIVE) @session_start();
    unset($_SESSION['gbdbui_cache']);
}

/** Liefert rekursiv die logische Größe eines Ordners mit kurzem UI-Cache. */
...
function gbdbui_copy_tree(string $from, string $to): array {
    ...
    if (!is_dir($from)) return ['ok' => false, 'files' => 0, 'errors' => ['Quelle fehlt: ' . $from]];
    ...
    foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($from, FilesystemIterator::SKIP_DOTS), RecursiveIteratorIterator::SELF_FIRST) as $item) {
        ...
        if ($item->isDir()) { if (!is_dir($target)) @mkdir($target, 0777, true); continue; }
        if (!@copy($item->getPathname(), $target)) { $report['ok'] = false; $report['errors'][] = 'Konnte nicht kopieren: ' . $item->getPathname(); } else { $report['files']++; }
    }
    return $report;
}

/** Entfernt einen Ordnerinhalt rekursiv. */
