    ...
    <main class="gbdbui-wide">
        ...
        <section class="enterprise-hero-shell">
            <div class="enterprise-hero-copy">
                ...
                <p>Intranet-Pattern, Tenant-Grenzen, Quotas, Monitoring, Import/Export und Doku an einer Stelle – sauber
                    gebündelt für größere Setups.</p>
            ...
            <div class="enterprise-hero-status">
                ...
                <p><?= $quotaOk ? 'Quota-System meldet keine harten Limits.' : 'Quota-System meldet aktive Limits/Warnungen.' ?>
                </p>
            </div>
        </section>
        ...
        <section class="enterprise-action-grid">
            <article class="enterprise-action-card enterprise-card-glow">
                <div class="enterprise-card-head">
                    ...
                    <div>
                        ...
                        <p>Wählt ein JSON-Pattern aus <code>gbdb_framework/json/patterns</code>, erstellt eine neue
                            Instanz und legt darin Bases, Tabellen und optionale Datentypen an.</p>
                    </div>
                </div>
                    ...
                    <label>Pattern
                        <select name="pattern">
                            <?php foreach ($patterns as $pattern):
                                ...
                                if (empty($pattern['active']))
                                    continue; ?>
                                <option value="<?= gbdbui_e((string) $pattern['name']) ?>">
                                    <?= gbdbui_e((string) $pattern['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </label>
            ...
            <article class="enterprise-action-card">
                ...
                <form method="post" class="enterprise-quota-grid">
                    ...
                    <label>Max Rows/Table<input name="max_rows_per_table" type="number"
                            value="<?= (int) ($quota['limits']['max_rows_per_table'] ?? 100000) ?>"></label>
                    <label>Max Storage/Instance Bytes<input name="max_storage_per_instance" type="number"
                            value="<?= (int) ($quota['limits']['max_storage_per_instance'] ?? 1073741824) ?>"></label>
                    <label>Max API Calls<input name="max_api_calls" type="number"
                            value="<?= (int) ($quota['limits']['max_api_calls'] ?? 100000) ?>"></label>
                    <label class="checkline enterprise-check"><input name="enforce" type="checkbox"
                            <?= !empty($quota['limits']['enforce']) ? 'checked' : '' ?>> Limits aktiv erzwingen</label>
            ...
            <article class="enterprise-action-card">
                <div class="enterprise-card-head slim">
                    ...
                    <div>
                        ...
                        <p>Pattern erstellen, bearbeiten, aktivieren/deaktivieren oder löschen. Die Struktur folgt
                            <code>name</code>, <code>structure</code>, <code>base</code>, <code>tables</code>,
                            <code>useDataTypes</code> und <code>rows</code>.</p>
                    </div>
                </div>
                <div class="table-scroll enterprise-table-scroll">
                    <table class="enterprise-pattern-table">
                        ...
                        <tbody>
                            <?php foreach ($patterns as $pattern): ?>
                                <?php $patternName = (string) $pattern['name'];
                                $fileName = (string) ($pattern['_file'] ?? ($patternName . '.json')); ?>
                                <tr>
                                    <td>
                                    ...
                                    <td><span
                                            class="status-pill <?= !empty($pattern['active']) ? 'done' : 'queued' ?>"><?= !empty($pattern['active']) ? 'aktiv' : 'inaktiv' ?></span>
                                    </td>
                                    <td>
                                        <div class="enterprise-pattern-actions">
                                            <button type="button" class="secondary enterprise-edit-pattern"
                                                data-pattern="<?= gbdbui_e($patternName) ?>">Bearbeiten</button>
                                            <form method="post" class="inline-form enterprise-rename-form">
                                                ...
                                                <input name="new_pattern_name" value="<?= gbdbui_e($patternName) ?>"
                                            ...
                                            <form method="post" class="inline-form">
                                                ...
                                                <input type="hidden" name="active"
                                                    value="<?= !empty($pattern['active']) ? '0' : '1' ?>">
                                                <button class="secondary" name="enterprise_action"
                                                    value="toggle_pattern"><?= !empty($pattern['active']) ? 'Deaktivieren' : 'Aktivieren' ?></button>
                                            </form>
                                            <form method="post" class="inline-form">
                                                <input type="hidden" name="csrf"
                                                    value="<?= gbdbui_e(GreenQLUIv2Helper::csrf()) ?>">
                                                <input type="hidden" name="pattern" value="<?= gbdbui_e($patternName) ?>">
                                                <button class="danger" name="enterprise_action" value="delete_pattern"
                                                    onclick="return confirm('Pattern wirklich löschen?')">Löschen</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            <?php if (empty($patterns)): ?>
                                <tr>
                                    <td colspan="3" class="muted">Keine Patterns gefunden.</td>
                                </tr><?php endif; ?>
                        </tbody>
                    </table>
                </div>
            ...
            <article class="enterprise-action-card">
                <div class="enterprise-card-head slim">
                    ...
                    <div>
                        ...
                        <p>JSON einfügen und speichern. Bestehende Patterns werden anhand von <code>name</code>
                            überschrieben.</p>
                    </div>
                </div>
                <form method="post" class="stack-form compact-form enterprise-form-row"
                    id="enterprise-pattern-editor-form">
                    <input type="hidden" name="csrf" value="<?= gbdbui_e(GreenQLUIv2Helper::csrf()) ?>">
                    ...
                    <div class="gbdbui-editor-head enterprise-editor-head">
                        ...
                        <div>
                            <button type="button" class="gbdbui-mini-btn"
                                id="enterprise-format-json">Auto-Einrücken</button>
                            <button type="button" class="gbdbui-mini-btn" id="enterprise-reset-template">Template
                                laden</button>
                        </div>
                    </div>
                    <div class="gbdbui-highlight-editor enterprise-json-editor" data-lang="json">
                        <pre aria-hidden="true"></pre>
                        <textarea id="enterprise-pattern-json" name="pattern_json" rows="18" spellcheck="false"
                            autocomplete="off"><?= gbdbui_e($patternTemplateJson) ?></textarea>
                    </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <?php if (empty($tableRows)): ?>
                                <tr>
                                    <td colspan="2" class="muted">Keine Tabellen gefunden.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
